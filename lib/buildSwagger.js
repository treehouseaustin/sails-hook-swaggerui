...
module.exports = function(apiEntries, callback){
  ...
  async.each(apiEntries, function(oneFile, next){
    ...
    var flattenedResults = _.flatten(_.compact(_.map(_.flatten(_.map(parseResults.success, 'docs')), 'tags')));

    // Create an object where the key names are API paths and the values are
    // arrays of objects.  Each object is the documentation for one particular
    // HTTP method associated with that endpoint.
    flattenedResults = _.reduce(flattenedResults,function(keepers,oneResult){

      // API path (i.e. `/user/pets`)
      var pathName = _.keys(oneResult)[0];

      var resultObject = oneResult[pathName];

      // HTTP method name (i.e. `POST`)
      var methodName = _.keys(oneResult[pathName])[0];

      if (!_.keys(keepers[pathName]).length){
        keepers[pathName] = {};
      }

      keepers[pathName][methodName] = oneResult[pathName][methodName];
